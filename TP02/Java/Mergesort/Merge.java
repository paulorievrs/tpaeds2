...
class MergeSort extends MetodosLista {
...
{ 
    ...
    for (i = 0; i < n1; i++) 
		L[i] = personagemA[l + i].clone(); 
		
    for (j = 0; j < n2; j++) 
        R[j] = personagemA[m + 1+ j].clone(); 
    ...
    { 
        ...
        { 
            personagemA[k] = L[i].clone(); 
            i++; 
        } 
        else
        { 
            personagemA[k] = R[j].clone(); 
            j++; 
        } 
        k++; 
    } 
...
public class Merge {
	static MergeSort lista = new MergeSort(1000);
