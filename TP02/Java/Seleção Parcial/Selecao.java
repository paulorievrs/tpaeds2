...
class Selection extends MetodosLista {
	...
	public void SelecaoParcial(int k) {
		...
		for(i = 1; i <= k; i++) {
			min = i;
			for(j = i + 1; j < n; j++) {
				if (personagemA[j].getNome().compareTo(personagemA[min].getNome()) < 0) {
					min = j;
				} 
				swap(min, i);
			}
...
public class Selecao {
	static Selection lista = new Selection(1000);
	...
	public static void setPersonagem(String strArquivo, Personagem personagem) throws Exception{
     ...
     public static void escreverNoArquivo(String conteudo) throws Exception{
		try {
			File diretorio = new File("655985_selecao.txt");
			FileWriter arq = new FileWriter("655985_selecao.txt");
			arq.write(conteudo);
			arq.close();
