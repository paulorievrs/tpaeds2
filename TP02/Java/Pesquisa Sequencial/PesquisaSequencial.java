...
public class PesquisaSequencial {
    ...
    public static Boolean pesquisarPersonagem(String nome) throws Exception{
    	Boolean temOnome = false;
		Personagem pesquisa[] = lista.getPersonagemA();
		Personagem aux = new Personagem();

    	for(int i = 0; i < lista.getN(); i++) {
    		aux = pesquisa[i].clone();
	...
	public static void escreverNoArquivo(String conteudo) throws Exception{
		try {
			File diretorio = new File("matricula_sequencial.txt");
	...
	public static void main(String args[]) throws Exception{
		...
		String[] localizacaoArquivos = new String[1000];
		String veioDoArquivo = new String();
		int quantidadeEntrada = 0;

        //inicio leitura de dados
		do {

			localizacaoArquivos[quantidadeEntrada] = MyIO.readLine();

		} while (isFim(localizacaoArquivos[quantidadeEntrada++]) == true);
		quantidadeEntrada--;
		...
		for(int i = 0; i < quantidadeEntrada; i++) {
			...
			String convertida = new String(localizacaoArquivos[i].getBytes("ISO-8859-1"), "UTF-8");
			veioDoArquivo = arquivo[i].lerArquivo(convertida);
			setPersonagem(veioDoArquivo, personagem[i]);
			lista.inserirFim(personagem[i]);
		}


		String[] nomesPersonagens = new String[1000];
		int quantidadePersonagens = 0;

		do {

			nomesPersonagens[quantidadePersonagens] = MyIO.readLine();

		} while (isFim(nomesPersonagens[quantidadePersonagens++]) == true);
		//fim leitura de dados
		quantidadePersonagens--;
		Boolean temOnome = true;

		for(int i = 0; i < quantidadePersonagens; i++) {
			temOnome = pesquisarPersonagem(nomesPersonagens[i]);
			if(temOnome == true) {
				qtdComparacoes++;
				MyIO.println("SIM");
			} else {
		...
		String conteudo = matricula + "\t" + tempo + "\t" +qtdComparacoes;
