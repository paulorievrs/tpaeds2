...
class Quick extends MetodosLista {
	...
	public void quick (int esq, int dir, int k) {
		int i = esq;
		int j = dir;
		Personagem pivo = personagemA[(dir+esq)/2].clone();

		while (i <= j) {
			while ((personagemA[i].getCorDoCabelo().compareTo(pivo.getCorDoCabelo()) < 0) || (personagemA[i].getCorDoCabelo().compareTo(pivo.getCorDoCabelo()) == 0 && personagemA[i].getNome().compareTo(pivo.getNome()) < 0)) {
				i++;
			}
			while ((personagemA[j].getCorDoCabelo().compareTo(pivo.getCorDoCabelo()) > 0) || (personagemA[j].getCorDoCabelo().compareTo(pivo.getCorDoCabelo()) == 0 && personagemA[j].getNome().compareTo(pivo.getNome()) > 0)) {
				j--;
			}
...
public class Quicksort {
	static Quick lista = new Quick(1000);
	...
	public static void setPersonagem(String strArquivo, Personagem personagem) throws Exception{
     ...
     public static void escreverNoArquivo(String conteudo) throws Exception{
		try {
			File diretorio = new File("655985_quicksort.txt");
			FileWriter arq = new FileWriter("655985_quicksort.txt");
			arq.write(conteudo);
			arq.close();
