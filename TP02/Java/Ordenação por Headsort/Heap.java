...
class Heapsort extends MetodosLista {
	...
	public void constroi(int tamHeap){
      for(int i = tamHeap; i > 1; i /= 2){
      	if (personagemA[i].getAltura() > personagemA[i/2].getAltura()) {

         	swap(i, i/2);

      	} else if (personagemA[i].getAltura() == personagemA[i/2].getAltura()) {

      		if ((personagemA[i].getNome().compareTo(personagemA[i/2].getNome()) > 0)) {
         			swap(i, i/2);
         }
      	}       	
      }
   ...
   public void swap(int i, int j) {
		Personagem temp = personagemA[i].clone();
		personagemA[i] = personagemA[j].clone();
		personagemA[j] = temp.clone();
		quantidadeDeMovimentacoes+=3;
	}
  ...
  public void reconstroi(int tamHeap){
      ...
      while(i <= (tamHeap/2)){
         ...
         if (personagemA[2*i].getAltura() > personagemA[2*i+1].getAltura() || 2*i == tamHeap){
            filho = 2*i;
         } else if(personagemA[2*i].getAltura() == personagemA[2*i*+1].getAltura() ) {
         	if((personagemA[2*i].getNome().compareTo(personagemA[2*i+1].getNome()) > 0)) {
         		filho = 2*i;
         ...
         if(personagemA[i].getAltura() < personagemA[filho].getAltura()){
            swap(i, filho);
            i = filho;
         } else{
            i = tamHeap;
	...
	public void heapsort() {
      ...
      for(int i = 0; i < n; i++){
         tmp[i+1] = personagemA[i].clone();
      }
      personagemA = tmp.clone();
      ...
      tmp = personagemA;
      personagemA = new Personagem[n];
      for(int i = 0; i < n; i++){
         personagemA[i] = tmp[i+1];
      }
...
public class Heap {
	static Heapsort lista = new Heapsort(1000);
	...
	public static void setPersonagem(String strArquivo, Personagem personagem) throws Exception{
     ...
     public static void escreverNoArquivo(String conteudo) throws Exception{
		try {
			File diretorio = new File("655985_headpsort.txt");
			FileWriter arq = new FileWriter("655985_.txt");
			arq.write(conteudo);
			arq.close();
