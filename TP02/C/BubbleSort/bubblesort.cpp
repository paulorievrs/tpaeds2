...
void bubbleSort() {
   for (int i = (n - 1); i > 0; i--) {
      for (int j = 0; j < i; j++) {
         if (strcmp(personagemA[j].anoNascimento, personagemA[j + 1].anoNascimento) > 0) {
            Personagem auxiliar = personagemA[j + 1];
            personagemA[j + 1] = personagemA[j];
            personagemA[j] = auxiliar;
         }
      }
   }
}
